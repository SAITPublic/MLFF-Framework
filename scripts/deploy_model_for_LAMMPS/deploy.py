...
if sys.argv[1] == "NequIP":
    ...
    raise RuntimeError("other models are not supported")
...
if 'chemical_symbols' in ckpt['config']['model_attributes'].keys():
    ...
    vals = [ckpt['config']['model_attributes']['chemical_symbol_to_type'][sym] for sym in symbols]
    idx = np.argsort(vals)
    sym_array = np.array(symbols)
    type_names = list(sym_array[idx])
