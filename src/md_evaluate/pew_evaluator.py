...
class PEWEvaluator(BaseEvaluator):        
    ...
    def calculate_pe_well_error_metric(df_ref_in, df_mlff_in, res_out_path):
        ...
        print("[mlff model] min_dist_list: {}, min_PE_list: {}".format(min_dist_list, min_PE_list))
    ...
    def load_reference_results(file_path):
        ...
        print("Number of data points for pe_well (reference): {}".format(df_ref.shape[0]))
        return df_ref
    ...
    def evaluate(self):
        ...
        for unit_structure_dict in self.config["unit_structures"]:
            ...
            df_ref = self.load_reference_results(unit_structure_dict["reference_scale_energy_relation_path"])
            
            structure_error_save_name = "{}_{}.txt".format(self.config["error_save_name"],
                                                           structure_name)
            PEWEvaluator.calculate_pe_well_error_metric(df_ref,
                                       df_mlff,
                                       out_dir / structure_error_save_name)
        
            PEWEvaluator.generate_comparison_plot(df_ref, 
                            df_mlff, 
