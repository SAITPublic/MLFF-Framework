...
@md_evaluate_registry.register_md_evaluate("pe_curves")
@md_evaluate_registry.register_md_evaluate("potential_energy_curves")
class PECEvaluator(BaseEvaluator):

    def calculate_pe_curves_error_metric(self, df_ref_in, df_mlff_in, res_out_path):
    ...
    def generate_comparison_plot(self, df_ref, df_mlff, fig_out_path, save_res):
        ...
        if save_res:
            ...
            self.logger.info(f"Figure to compare PE Curves was saved in {fig_out_path}")
    ...
    def load_reference_results(self, file_path):
        ...
        self.logger.info(f"Number of data points for pe_curves (reference): {df_ref.shape[0]}")
        return df_ref
    ...
    def evaluate(self):
        ...
        for unit_structure_dict in self.config["unit_structures"]:
            ...
            if unit_structure_dict.get("interatomic_distances"):
                ...
                for dist in PECEvaluator.construct_range_array(unit_structure_dict["interatomic_distances"]):
                    try:
                ...
                data = defaultdict(list)
                for scale_suffix in PECEvaluator.construct_range_array(unit_structure_dict["filename_scale_suffixes"]):
            ...
            self.calculate_pe_curves_error_metric(
                df_ref,
                df_mlff,
            ...
            self.generate_comparison_plot(
                df_ref,
                df_mlff,
                out_dir / f"compare_pe_curves_{structure_name}.png",
                save_res=True
            )
