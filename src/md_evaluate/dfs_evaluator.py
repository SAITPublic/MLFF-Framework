...
class DFEvaluator(BaseEvaluator):
    ...
    def generate_comparison_figure(self, distribution_ref, distribution_dict_mlff, combination_list, fig_name):
        ...
        for combination in combination_list:
            ...
            plt.savefig(Path(self.config["res_out_dir"]) / '{}_{}.png'.format(fig_name, combination))
            # plt.show()
            # # plt.close('all')
    ...
    def get_traj_atoms(path, index, format, n_extend):
        ...
        if n_extend is not None and n_extend != 1:
            ...
            for atoms in traj_atoms:
                atoms.wrap()
                atoms_extended = make_supercell(atoms, [[n_extend,0,0],[0,n_extend,0],[0,0,n_extend]], wrap=True)
    ...
    def evaluate(self):
        ...
        trajs_atoms_ai = DFEvaluator.get_traj_atoms(self.config["ai_md_traj"]["path"], 
        ...
        out_identifier_ai = "AIMD_".format(self.config["ai_md_traj"]["out_identifier"])
        ...
        for mlff_uid, mlff_traj_dict in self.config["mlff_md_traj"].items():
            traj_atoms_mlff = \
                DFEvaluator.get_traj_atoms(mlff_traj_dict["path"], 
