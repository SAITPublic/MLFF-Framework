...
class EoSEvaluator(BaseEvaluator):            
    ...
    def calculate_eos_error(self, eos_fit_ref, eos_fit_mlff, save_res =True):
        ...
        if save_res:
            res_df.to_csv(Path(self.config["res_out_dir"]) / "eos_error_metrics.csv")
    ...
    def load_reference_results(self, file_path, scale_factors, eos_type):
        ...
        df = df[range_mask]
        print("Number of data points to consider for eos (reference): {}".format(df.shape[0]))    
    ...
    def evaluate(self):
        ...
        for scale_factor in scale_factors:
            ...
            atoms = copy.deepcopy(atoms_no_scale)   ## check if the calculator of atoms_no_scale is also copied
        ...
        if self.config["reference_result"]["save_eos_fit"]:
            save_path_eos_ref = save_path_eos = Path(self.config["reference_result"]["dir"]) / eos_res_name
